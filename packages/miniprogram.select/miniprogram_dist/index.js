...
const getNotFoundContent = (v)=>{
    if (v !== null && typeof v === 'object') return Object.assign({}, notFoundContent, v);
    if (typeof v === 'string') return Object.assign({}, notFoundContent, {
        text: v
    });
...
const getSelectIndex = (options, value, multiple)=>{
    if (multiple) {
        ...
        return options.map((item, index)=>({
                item,
                index
            })).filter(({ item })=>!item.isGroup && values.includes(item.value)).map(({ index })=>index);
    }
...
let Select = class Select extends Doraemon {
    ...
    runCallbacks(method, detail) {
        ...
        if (typeof fn === 'function') fn.call(this, detail.value, index, mergedOptions);
    }
    onConfirm(e) {
    ...
    constructor(...args){
        ...
        this.selectMax = -1;
        this.selectNotFoundContent = {
            ...notFoundContent
        };
        this.selectVirtualized = false;
        this.selectToolbar = {
            title: '请选择',
            cancelText: '取消',
            confirmText: '确定'
...
var index = defineComponentHOC()(Select);
