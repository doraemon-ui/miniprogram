...
let Tabs = class Tabs extends Doraemon {
    ...
    get classes() {
        ...
        const prev = classNames([
            `${prefixCls}__mask`,
            `${prefixCls}__mask--prev`
        ]);
        const next = classNames([
            `${prefixCls}__mask`,
            `${prefixCls}__mask--next`
        ]);
        return {
            wrap,
            scrollView,
    ...
    async setNextScroll(activeElement) {
        ...
        if (!scroll) return;
        const [container, activeTab] = await Promise.all([
...
var index = defineComponentHOC()(Tabs);
