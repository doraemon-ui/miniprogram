...
function getRealCols(data = [], fieldNames = DEFAULT_FIELD_NAMES) {
    const cols = isMultiPicker(data) ? data : [
        data
    ];
    return cols.reduce((acc, col)=>[
            ...acc,
            getRealCol(col, fieldNames)
        ], []);
}

export { getIndexFromValue, getIndexesFromValues, getLabelFromIndex, getLabelsFromIndexes, getRealCol, getRealCols, getRealIndex, getRealValue, getRealValues, isMultiPicker };
