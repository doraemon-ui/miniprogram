...
let SelectorGroup = class SelectorGroup extends Doraemon {
    ...
    get optionsView() {
        const names = this.defaultFieldNames || {};
        ...
        return (this.options || []).map((item)=>{
            if (typeof item === 'string') {
                return {
                    ...
                    disabled: false,
                    desc: '',
                    checked: this.inputValue.includes(item)
                };
            }
    ...
    getValue(value = this.inputValue) {
        ...
        const checkedValues = value.reduce((acc, val)=>{
            return [
                ...acc,
                ...cols.filter((option)=>option.value === val).map((option)=>({
                        label: option.label,
                        value: option.value
                    }))
            ];
    ...
    onCheckboxChange(e) {
        ...
        const checkedValues = this.multiple ? this.inputValue.includes(value) ? this.inputValue.filter((n)=>n !== value) : [
            ...this.inputValue,
            value
        ] : checked ? [
            value
...
var index = defineComponentHOC()(SelectorGroup);

export { SelectorGroup, index as default };
