...
import { DATETIME, convertStringArrayToDate, ONE_DAY, cloneDate, DATE, YEAR, MONTH, TIME, isTillNow, fomartArray, getDaysInMonth, pad, TILL_NOW, setMonth, convertDateToStringArray, getRealValues, getIndexesFromValues, getLabelsFromIndexes } from './utils.js';
...
let DatePickerView = class DatePickerView extends Doraemon {
    ...
    updated(inputValue) {
        if (this.inputValue.join('|') !== inputValue.join('|')) this.inputValue = [
            ...inputValue
        ];
    }
    setValue(v) {
...
DatePickerView = _ts_decorate([
    Component({
        props
    })
], DatePickerView);
var index = defineComponentHOC()(DatePickerView);

export { DatePickerView, index as default };
