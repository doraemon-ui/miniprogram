...
function convertStringArrayToDate(value, props = {}) {
...
function convertDateToStringArray(date, props = {}) {
    if (!date) return [];
    const { mode, use12Hours } = props;
    let value = [];
    if (mode === YEAR) value = [
    ...
    if (mode === DATETIME || mode === DATE) value = [
        `${date.getFullYear()}`,
        `${date.getMonth()}`,
        `${date.getDate()}`
    ];
...
export { DATE, DATETIME, MONTH, ONE_DAY, TILL_NOW, TIME, YEAR, cloneDate, convertDateToStringArray, convertStringArrayToDate, fomartArray, getDaysInMonth, getIndexFromValue, getIndexesFromValues, getLabelFromIndex, getLabelsFromIndexes, getRealIndex, getRealValue, getRealValues, isTillNow, modeRecord, pad, setMonth };
