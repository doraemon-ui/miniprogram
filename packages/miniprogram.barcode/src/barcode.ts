...
import type { BarcodeCanvas2DContext as Canvas2DContext, BarcodeSettings } from './types'

export default class EAN13 {
  private canvas: WechatMiniprogram.Canvas
  private ratio: number
  private number: string
  private settings: BarcodeSettings

  constructor(canvas: WechatMiniprogram.Canvas, ratio: number, number: string, options?: Partial<BarcodeSettings>) {
    this.canvas = canvas
    this.ratio = ratio || 1
    this.number = number
    this.settings = {
  ...
  private generateCheckDigit(number: string) {
    let counter = 0
