...
export function initWatch(vm: Doraemon, watch: ComponentOptions<Doraemon>['watch']) {
  return Object.keys(watch).reduce<Record<string, (newVal: any) => void>>(
    (acc, key) => ({
      ...acc,
...
function createWatcher(vm: Doraemon, key: string, handler: WatchOptionsWithHandler<any> | WatchHandler<any>) {
  return function defineWatch(newVal: any) {
    ...
    const handlers: Array<WatchOptionsWithHandler<any> | WatchHandler<any>> = Array.isArray(handler) ? handler : [handler]
