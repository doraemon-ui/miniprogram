...
export function initRefs(vm: Doraemon) {
  ...
  Object.defineProperty(vm, '$parent', {
    get() {
      const nodes = parentNodes.slice(0, 1).reduce<ComponentPublicInstance[]>((acc, path) => [...acc, ...find(vm, path)], [])
  ...
  Object.defineProperty(vm, '$children', {
    get() {
      const nodes = childrenNodes.reduce<ComponentPublicInstance[]>((acc, path) => [...acc, ...find(vm, path)], [])
      return nodes
    },
  })
  Object.defineProperty(vm, '$refs', {
    get() {
      const nodes = refNodes.reduce<{ [key: string]: ComponentPublicInstance | ComponentPublicInstance[] }>(
        (acc, node) => ({
          ...acc,
